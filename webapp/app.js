...
async function generate() {
  ...
  try {
    ...
    // 2. Вставка основного текста
    const fullText = buildGuideText(data, lang);
    setText("guide", fullText); // Самое важное место!
    ...
    if (pl) {
      ...
      if (data.map_points && data.map_points.length > 0) {
        ...
        const li = document.createElement("li");
        li.textContent = lang === "ru" ? "Нет точек" : "No points";
        pl.appendChild(li);
      }
    }
